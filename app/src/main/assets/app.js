...
// ===== FINAL OFFLINE FIX CORE (v2) =====

function ensureHydration(){
  state = state || {};
  state.beneficiaries = Array.isArray(state.beneficiaries) ? state.beneficiaries : [];
  ...
  state.bankAccounts = Array.isArray(state.bankAccounts) ? state.bankAccounts : [];
}
...
function importBeneficiaries(list){
  state.beneficiaries = Array.isArray(list) ? list : [];
  saveState();
}

function saveAttachmentFixed(file){
  if(!file) return;
...
// ===== MONTHLY ARCHIVE SYSTEM (OFFLINE PRO) =====
...
function ensureMonthly(){
  ...
  return state.monthlyData[m];
}

function addBeneficiary(item){
  const month = ensureMonthly();

  month.beneficiaries.push({
...
function getCurrentBeneficiaries(){
  return ensureMonthly().beneficiaries;
}
...
// ===== FINAL CONNECTED DATA FIX (CRITICAL) =====
...
function ensureMonthly(){
  ...
  state.monthlyData = state.monthlyData || {};
  if(!state.monthlyData[m]){
    state.monthlyData[m] = { beneficiaries: [], payments: [], attachments: [] };
  }
  return state.monthlyData[m];
}

// MIGRATION FIX: move old 658 beneficiaries into monthly system
function migrateLegacyBeneficiaries(){
  state.monthlyData = state.monthlyData || {};
  const m = getCurrentMonth();

  if(state.beneficiaries && state.beneficiaries.length){
    state.monthlyData[m] = state.monthlyData[m] || { beneficiaries: [], payments: [], attachments: [] };

    state.monthlyData[m].beneficiaries.push(...state.beneficiaries);

    state.beneficiaries = []; // prevent duplication
    saveState();
  }
}

// FIXED ADD (always writes to monthly bucket)
function addBeneficiary(item){
  const month = ensureMonthly();
...
function importBeneficiaries(list){
  const month = ensureMonthly();
  month.beneficiaries = Array.isArray(list) ? list : [];
  saveState();
}

// AUTO RUN MIGRATION ON LOAD
migrateLegacyBeneficiaries();




// ===== EXPORT FINAL FIX (ALL MONTHS MERGE) =====

function getAllBeneficiariesForExport(){
  const data = state.beneficiariesByMonth || {};
  let all = [];

  Object.keys(data).forEach(month => {
    const list = data[month]?.beneficiaries || [];
    list.forEach(b => {
      all.push({
        ...b,
        month: month
...
function exportPayload(){
  return {
    ...state,
    beneficiaries: getAllBeneficiariesForExport(),
...
function getJalaliDate(){
  const d = new Date();
  const g = toJalali(d.getFullYear(), d.getMonth()+1, d.getDate());

  return `${g.jy}/${String(g.jm).padStart(2,'0')}/${String(g.jd).padStart(2,'0')}`;
}

// PATCH: enhance export with Jalali date
function enhanceWithJalali(item){
  return {
    ...item,
    createdAtJalali: getJalaliDate()
  };
}
